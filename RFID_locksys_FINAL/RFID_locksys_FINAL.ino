...
const String sheet_url_send = "https://script.google.com/macros/s/AKfycbzrh70kmpgb6LpuEw5dacFCccGHdu1FcQDzb_HRpPRYrF0VeBPX2YNXhAFZsLwS3JA8eA/exec?name=";//add ?name= at last of the url
...
void loop()
{
  ...
  if (response == "true") {
    ...
    tone(BUZZER, 400); // buzzer beep to inform that attendance registered
    delay(50);
    noTone(BUZZER);
    delay(500);
...
void checkifAuthorised() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    client->setInsecure();
    //-----------------------------------------------------------------
    card_holder_name_auth = sheet_url_auth + CardUID;
